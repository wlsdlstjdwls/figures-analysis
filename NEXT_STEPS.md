...
> 최종 업데이트: 2026-06-25(13) · **naver MMS 라인명시 검수 매칭 +23(비교그룹 67→74, 교차출처 52→59)**. 가동 **19소스**.
> 새 세션에서 이 파일부터 읽으면 이어서 작업 가능. 전체 설계는 [PLAN.md](PLAN.md).

## 이번 세션 변경 (2026-06-25 #13) — ⭐ naver 라인명시 검수 매칭 (#1b)

### export_review 덤프 정독 → naver MMS 매물 23건 정확버전 매칭
- 진단: `run.py group` 덤프 199앵커. 후보 분포 bunjang859·danawa361·amiami162·naver134·ebay114. bunjang/danawa는 대부분 캔디토이BOX vs 개별·제네릭(과거세션처럼 제외). **알짜=naver 무비몬스터시리즈(MMS) 매물**이 amiami/solaris MMS 앵커와 char+maker+line 일치 → 정밀 1:1 판정.
- 추출: `awk`로 naver 후보 보유 앵커블록만 분리(scratchpad `naver_only.txt`). 같은 괴수 다앵커(reissue 연식별) 케이스를 **버전 토큰으로 분리 매칭**.
- **확정 23건**(`assign_to_group`, method=manual:review → prune 보존):
  - 메카고질라 **중무장형**(=Heavy Armament=2003 ver.2.0) → FIGURE-189115 ×4 (35858446984·36271810223·58825598791·58845655000). ⚠️2002/2021/1993/EX는 중무장형 아님→배제.
  - 데스토로이아 → FIGURE-031643 ×2. 스페이스고질라 → FIGURE-043326 ×3.
  - 안기라스 **SP**(싱귤러포인트) → FIGURE-121812 ×2 / **고지반** → FIGURE-140770 ×1.
  - 제트재규어 **베타** → FIGURE-132966 / 일반SP → FIGURE-121809.
  - 헤도라1971 → FIGURE-132965. 비오란테 → FIGURE-193442 ×6. 제트재규어(vs메가로) → FIGURE-121815. 엔스카이 퍼펫마스코트 **시리즈2** → GOODS-04330781.
- **제외 패턴**(보수): ① 제네릭 라돈(쇼와/2019 다앵커 모호) ② 고지반/파이어라돈/치비기도라=다른 제품 ③ 가메라2023·마스코트3=앵커없음 ④ 미니라 번들(3종/5종). 사양 불명확 1건(바트라 "2024진화G4")도 배제.
- 검증: assign 23건 적재, naver/앵커 전부 최신스냅샷 존재 확인. **비교그룹 67→74, 교차출처 52→59**. naver manual:review 총30(이번23+#8/#11 7). 대시보드 19,683건 재생성.
- 🎯 **다음**: ① 남은 검수덤프 후보=bunjang/danawa 제네릭·캔디토이BOX 위주(진짜매칭 희소, 저ROI). ② 제네릭 라돈 등 다앵커 모호건 = grouping에 "버전무표기→base앵커" 정책 도입하면 recall↑(precision tradeoff, 사용자 판단). ③ 같은라인 다른서브괴수 분리(저영향).
