...
| 한국 | **WYYYES(와이스)** | SPA XHR 역분석 / Playwright | ★★★ **낙찰가=실거래(국내)** | 중상 | **국내 실거래 앵커 후보**. 컬렉터 특화·피규어 카테고리. §10.5 |
| 한국 | **번개장터** | 비공식 API (find_v2.json) | ★★ 중고 호가 | **낮음(실측)** | ✅ **연동완료**. 인증불필요·이미지·등록일·지역. §10.6 |
| 한국 | 중고나라 | 스크랩 / 카페 | ★ 중고 호가 | 높음 | 물량 최대지만 노이즈·구조 불규칙 |
...
- **실거래 데이터 부족 (rev.2 격상 → rev.4 완화 후보)**: eBay sold = Marketplace Insights 제한 API, 개인 확보 어려움. 야후옥션 API 경로 폐쇄(§10.3). → **국내 WYYYES(와이스) 경매 낙찰가가 국내 실거래 앵커 후보로 부상(§10.5).** 단 비공식 XHR 역분석·ToS 확인 필요. Surugaya 시세는 백업 앵커.
...
### 10.6 추가 소스 실측 (rev.5, 2026-06-24)
- **번개장터 ✅**: `api.bunjang.co.kr/api/1/find_v2.json?q=&page=&n=100` — 인증 불필요 공개 JSON.
  pid·name·price·product_image·update_time(등록일 unix)·location(지역)·num_faved. 첫 수집 880여건.
  난이도 예상(높음)과 달리 **실측 쉬움**. 저빈도·딜레이 유지(차단 회피). 상세: m.bunjang.co.kr/products/<pid>.
- **출시일(발매일)**: 네이버 쇼핑 API엔 날짜 필드 없음. JP 발매일 소스(아미아미·스루가야)는 아래대로 차단.
  → 대안: **제목 내 연식(예: 고질라(1989)) 정규식 추출**로 빈티지/원형 연도 식별(대시보드 연식 배지). 정확 발매일은 eBay/만다라케/JP 인프라 확보 후.
- **해외/JP 크롤링 검토 (실측)**:
  | 사이트 | 접근 | 결과 | 비고 |
  |--------|------|------|------|
...
- **거래 방식**: ① 라이브 즉시경매(30초~3분) ② 72시간 경매 ③ 즉시판매. → **경매 낙찰가 = 국내 실거래가 확보 가능.** 야후 폐쇄·eBay sold 게이트로 비어있던 **실거래 앵커를 국내 시장에서 메우는 핵심 후보.**
- **카테고리/URL**: 피규어 목록 `/categorytab?viewCategory=figure`, 상세 `/category/figure/<id>?viewCategory=figure`. 소프비/괴수 매물 존재(예: 괴수8호 피규어 확인).
- **robots.txt**: 개인화 경로 62개(`/user/`,`/orderlist`,`/direct-message-room` 등)만 Disallow. **카테고리/상품 조회는 허용.** sitemap.xml 제공. Baiduspider만 전면 차단.
- **접근 기술**: 공개 API 없음. SPA(첫 HTML 비어있음 → 클라이언트 렌더). 수집은 ⓐ 브라우저 XHR 역분석으로 내부 JSON 엔드포인트 확보(가벼움, 우선) 또는 ⓑ Playwright 렌더 스크랩. 난이도 중상.
- **리스크**: ToS 재확인 필요(거래 데이터 수집·재배포). 라이브 경매는 실시간성 → 낙찰 결과 폴링 설계 필요. 저빈도·소량 원칙(§3.1) 적용.
- **다음 액션**: DevTools Network 탭으로 figure 카테고리·낙찰내역 XHR 엔드포인트 식별 → 비공식 API 수집기 시작(Phase3 한국 중고 단계로 편입, 단 실거래 가치 높아 우선순위 격상 검토).
